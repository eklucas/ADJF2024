---
title: "Data cleaning with tidyr, stringr and lubridate"
output: 
---
...
disney <- disney %>% mutate(
  ...
  new_inflation_adjusted = parse_number(inflation_adjusted_gross)
)
```
...
```{r}
disney %>% arrange(desc(new_inflation_adjusted))

disney %>% arrange(new_dt)
```
...
# using head() to limit results, just for demonstration:
head(str_sub(poverty$GEOID, 1, 2))
...
One last string to manipulate here, and that is the `NAME` column. In the data we worked with eariler this week, I had separated out county and state names to make it easier to work with. I did this using the `separate()` function, which allows you to split a column into multiple columns based on a delimiter:
...
```{r}
poverty_wide %>% 
  mutate(below200_pct = below200/population) %>%
  select(state, county, below200, population, below200_pct) %>% 
  filter(county == "Boone County")
```
...
poverty_wide %>% 
  ...
  summarise(population = sum(population),
            below200 = sum(below200)) %>% 
  mutate(below200_pct = below200/population) %>% 
  arrange(desc(below200_pct))
```
